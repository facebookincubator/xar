...
class XarFile(object):
    ...
    def _read_header(self):
        ...
        for attr in required_attributes:
            if attr not in self.attributes:
                raise ValueError("Attribute %s missing from %s" %
                                 (attr, self.filename))
    ...
    def mount(self, xarexec):
        ...
        proc = subprocess.Popen([xarexec, "-m", self.filename],
                                stdout=subprocess.PIPE)
        stdout, _ = proc.communicate()
        if proc.returncode != 0:
            logger.fatal("Mount of %s failed, see stderr for details" %
                         self.filename)
            return False
...
def flock_with_timeout(lock_fd, lock_type, timeout_sec):
    ...
    try:
        ...
        if ie.errno != errno.EINTR:
            logging.error(
                "Unexpected signal during flock: %s" %
                ie.strerror
            )
        return False
    finally:
...
def should_unmount(devname, mountpath, fstype, timeout):
    if fstype not in ('fuse.squashfuse', 'fuse.squashfuse_ll',
                      'osxfusefs', 'osxfuse'):
        return (False, None)
    ...
    if not match:
        logger.info("Skipping unmount of %s, unexpected path strucure" %
                    mountpath)
        return (False, None)
    ...
    if not os.path.exists(stat_target):
        stat_target = os.path.join(
            os.path.dirname(mountpath), 'lockfile')
    ...
    if time.time() - st.st_mtime <= timeout * 60:
        logger.info("Skipping unmount of %s, too recent (%.2fs)" %
...
def linux_mounts():
    ...
    mounts = []
    mounts_files = set(
        os.path.realpath(s) for s in ['/proc/mounts', '/etc/mtab'])
...
def unmount(mounts, opts):
    for mount in mounts:
        ...
        do_it, lock_fd = should_unmount(
            devname, mountpoint, fstype, opts.timeout
        )
        if do_it:
...
def main(args):
    ...
    p.add_option("--xarexec", default="/bin/xarexec_fuse",
    ...
    p.add_option("--cleanup", action="store_true",
                 help="unmount unused XAR mountpoints")
    p.add_option("--verbose", action="store_true",
                 help="print debugging info")
    p.add_option("--timeout", type=int, default=15,
                 help="time, in minutes, after a xar was mounted to attempt "
                 "to unmount it")
    ...
    for filename in filenames:
        ...
        if (xar_file.alias not in xar_files or
              xar_file.version > xar_files[xar_file.alias].version):
