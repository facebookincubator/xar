...
def extract_python_archive_info(archive):
    ...
    with zipfile.ZipFile(archive) as zf:
        ...
        MAIN = "__main__"
        main_exists = any(
            xar_util.file_in_zip(zf, MAIN + ext) for ext in PYTHON_EXTS
        )
        if main_exists:
            return (python, MAIN)
        return (python, None)
...
class Wheel(object):
    ...
    def __init__(self, distribution=None, location=None):
        ...
        if distribution is not None:
            ...
            self.distribution = \
                pkg_resources.DistInfoDistribution.from_filename(
    ...
    def install(self, src_paths, dst_paths, force=False):
        ...
        Uses either :install_archive(): or :copy_installation():.
        """
    ...
    def _determine_kind(self, src_paths, dst_paths, src_record):
        ...
        for prefix in xar_util.yield_prefixes_reverse(src_record):
            if prefix in src_paths.values():
                kinds = [
                    kind for kind, path in src_paths.items() if prefix == path
                ]
                break
    ...
    def copy_installation(self, src_paths, dst_paths, force=False):
        ...
        dst_records_path = os.path.join(
            dst_root, self.distinfo_name(), self.RECORD
        )
    ...
    def fixup(self, install_paths):
        ...
        py_files = [
            ...
            for record, _, _ in records if record.endswith(".py")
        ]
        py_set = set(py_files)
        ...
        new_records += [
            ...
            for f in py_files if f not in errors
        ]
